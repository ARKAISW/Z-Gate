...
with tab4:
    ...
    st.caption("Midnight UTC automated trade review and health audits generated by Featherless / Groq LLMs.")

    col_btn, _ = st.columns([1, 3])
    with col_btn:
        if st.button("✨ Generate Reflection Journal Now", use_container_width=True):
            with st.spinner("Analyzing performance with Featherless / Groq LLM..."):
                try:
                    from src.agents.reflection_agent import ReflectionAgent
                    from src.persistence.db import create_database
                    _db = create_database(config.get("db_path", "data/trades.db"))
                    _agent = ReflectionAgent(db=_db, config=config)
                    _res = _agent.run_nightly_reflection()
                    st.success("Reflection generated successfully!")
                    st.rerun()
                except Exception as exc:
                    st.error(f"Reflection execution error: {exc}")
    ...
    if not reflections_df.empty:
        ...
        st.info("No nightly reflections generated yet. Click 'Generate Reflection Journal Now' above or wait for midnight UTC.")
